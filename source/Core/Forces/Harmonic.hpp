...
HarmonicBond::HarmonicBond(double _k, double _r0, bool _normalized)
    : m_k(_k)
    , m_r0(_r0)
...
inline std::optional<Utils::vec2d> HarmonicBond::force(
...
inline std::optional<double> HarmonicBond::energy(const Utils::vec2d& _dx) const
{
  const auto r = _dx.norm();
  const auto dr = r - m_r0;
